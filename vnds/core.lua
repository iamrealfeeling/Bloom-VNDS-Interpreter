    ...
    while true do
        ...
        if state.choices then
            doChoice(state, it)
                drawGame(state, it)
                screen.flip()
                pcall(function() LUA.screenshot(novel.root .. "saves/pure_frame.png", 144, 80) end)
        else
...
-------------------------------------------------------------------------------
-- System Save screen (PSP Native SaveData Dialog via save.systemSave)
-------------------------------------------------------------------------------
function doSave(novel, it, state)
    audio.stopAll()

    -- RenPSP style: draw clean frame without pause menu before screenshot
    pcall(function()
        screen.clear(Color.new(0, 0, 0))
        drawGame(state, it)
        screen.flip()
        LUA.screenshot("temp.png", 144, 80)
    end)

    local st = it:saveState()
    pcall(function()
        save.systemSave(novel.root, st, novel.title)
    end)

    pcall(function() System.removeFile("temp.png") end)

    if st.curMusic and st.curMusic ~= "" then
        pcall(function() it:playMusic(st.curMusic) end)
    end
    return
end

-------------------------------------------------------------------------------
-- System Load screen (PSP Native LoadData Dialog via save.systemLoad)
-------------------------------------------------------------------------------
function doLoad(novel, it, state)
    audio.stopAll()
    local st = nil
    pcall(function()
        st = save.systemLoad(novel.root)
    end)
    if st then
...
function doPause(novel, it, state, syncVisual)
    ...
    while true do
        ...
        elseif buttons.pressed(buttons.cross) then
            ...
            if a == "save" then
                -- Hide pause menu instantly, draw clean game frame and screenshot
                screen.clear(Color.new(0, 0, 0))
                drawGame(state, it)
                screen.flip()
                local cleanThumb = novel.root .. "saves/clean_save.png"
                pcall(function() LUA.screenshot(cleanThumb, 144, 80) end)

                audio.stopAll()
                local st = it:saveState()
                pcall(function()
                    save.systemSaveCustom(novel.root, st, novel.title, cleanThumb)
                end)
                pcall(function() System.removeFile(cleanThumb) end)
                if st.curMusic and st.curMusic ~= "" then
                    pcall(function() it:playMusic(st.curMusic) end)
                end
                return
            elseif a == "load" then
                ...
                if st then
                    ...
                    if ok then
                        ...
                        if syncVisual then syncVisual() end
                    end
                end
                return
            elseif a == "menu" then
