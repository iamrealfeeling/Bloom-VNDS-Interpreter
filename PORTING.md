...
Данное руководство описывает технические требования, структуру файлов, аудиоформаты и тонкости синтаксиса скриптов, необходимые для успешного переноса и портирования визуальных новелл VNDS в интерпретатор Bloom VNDS Interpreter для PlayStation Portable.
...
2. **Слишком большие изображения:**  
   Фоны и CG должны быть конвертированы под разрешение PSP (`480x272`). Возможно я в скорем времени напишу конвертер для этого, но в любом случае вы можете конвертировать самостоятельно.
3. **Отладка крахов:**  
...
## Ну и на последок
Всё что здесь описано касается новых портов VNDS и новелл с других движков. Старые VNDS порты новелл можно запускать в сухую, вам только нужно будет сконвертировать аудио, спрайты, фоны и CG(если есть) в правильный формат поддерживаемый интерпретатором.
